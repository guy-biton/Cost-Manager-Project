...
## 6. Collections Setup
You don't necessarily need to create the collections manually in Atlas. Mongoose will automatically create the `users`, `costs`, and `logs` collections the first time you insert documents into them.

**Remember:** When submitting the project, your database must be completely empty EXCEPT for a single document in the `users` collection:
```json
{
  "id": 123123,
  "first_name": "mosh",
...
```
You can manually insert this user by going to the **Collections** tab in Atlas, clicking **Insert Document** under your `users` collection, and pasting the JSON above.
